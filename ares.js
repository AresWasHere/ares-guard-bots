...
const ares1 = require("discord.js");
const ares2 = require("discord.js");
const ares3 = require("discord.js");

const client1 = new ares1.Client();
const client2 = new ares2.Client();
const client3 = new ares3.Client();

const ayarlar = require('./ayarlar.json');
const config = require('./ares.json');
...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.roles.cache.forEach(async function(ares) {
  ...
  let channel = client1.channels.cache.get(ayarlar.defenderlog)
   ...
   const ares = new Discord.MessageEmbed()
   .setTimestamp()
  ...
  channel.send(`@everyone`, {embed: ares}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.roles.cache.forEach(async function(ares) {
  if (ares.permissions.has("ADMINISTRATOR") || ares.permissions.has("BAN_MEMBERS") || ares.permissions.has("MANAGE_GUILD") || ares.permissions.has("KICK_MEMBERS") || ares.permissions.has("MANAGE_ROLES") || ares.permissions.has("MANAGE_CHANNELS")) {
    ares.setPermissions(0).catch(err =>{});}});     
...
const entry = await member.guild
  ...
  if (member.user.bot) {
  member.guild.roles.cache.forEach(async function(ares) {
  if (ares.permissions.has("ADMINISTRATOR") || ares.permissions.has("BAN_MEMBERS") || ares.permissions.has("MANAGE_GUILD") || ares.permissions.has("KICK_MEMBERS") || ares.permissions.has("MANAGE_ROLES") || ares.permissions.has("MANAGE_CHANNELS")) {
    ares.setPermissions(0).catch(err =>{});}});
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.roles.cache.forEach(async function(ares) {
  if (ares.permissions.has("ADMINISTRATOR") || ares.permissions.has("BAN_MEMBERS") || ares.permissions.has("MANAGE_GUILD") || ares.permissions.has("KICK_MEMBERS") || ares.permissions.has("MANAGE_ROLES") || ares.permissions.has("MANAGE_CHANNELS")) {
    ares.setPermissions(0).catch(err =>{});}});
...
client2.on("roleDelete", async role => {
  ...
  role.guild.roles.cache.forEach(async function(ares) {
  if(ares.permissions.has("ADMINISTRATOR") || ares.permissions.has("BAN_MEMBERS") || ares.permissions.has("MANAGE_GUILD") || ares.permissions.has("KICK_MEMBERS") || ares.permissions.has("MANAGE_ROLES") || ares.permissions.has("MANAGE_CHANNELS")) {
    ares.setPermissions(0).catch(err =>{});}});  

  role.guild.members.ban(entry.executor.id, { reason: `Ares System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Ares System | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: "Ares System | Rol Koruma Sistemi" }).catch(e => { })	
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.roles.cache.forEach(async function(ares) {
  if(ares.permissions.has("ADMINISTRATOR") || ares.permissions.has("BAN_MEMBERS") || ares.permissions.has("MANAGE_GUILD") || ares.permissions.has("KICK_MEMBERS") || ares.permissions.has("MANAGE_ROLES") || ares.permissions.has("MANAGE_CHANNELS")) {
    ares.setPermissions(0).catch(err =>{});}});  
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Ares System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Ares System | Kanal Korum Sistemi!" }).then(async kanal => {
...
channel2.send(`@everyone`, {embed: ares}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.cache.roles.cache.forEach(async function(ares) {
  if(ares.permissions.has("ADMINISTRATOR") || ares.permissions.has("BAN_MEMBERS") || ares.permissions.has("MANAGE_GUILD") || ares.permissions.has("KICK_MEMBERS") || ares.permissions.has("MANAGE_ROLES") || ares.permissions.has("MANAGE_CHANNELS")) {
    ares.setPermissions(0).catch(err =>{});}});
...
client3.on("emojiDelete", async (emoji, message) => {
  ...
  channel.send(`@everyone`, {embed: ares}).catch(err => { })
return;
});
...
console.log(`[ARES] ${client1.user.username} ismi ile giriş yapıldı! Guard I Online`);
...
console.log(`[ARES] ${client2.user.username} ismi ile giriş yapıldı! Guard II Online`);
...
console.log(`[ARES] ${client3.user.username} ismi ile giriş yapıldı! Guard III Online`);
client3.user.setPresence({ activity: { name: ayarlar.botdurum }, status: ayarlar.status });
});  
client3.on("ready", async () => {
  let botVoiceChannel = client3.channels.cache.get(ayarlar.botVoiceChannelID);
  if (botVoiceChannel) botVoiceChannel.join().catch(err => console.error("Bot ses kanalına bağlanamadı!"));
});
